...
function initClick() {
    ...
    $(".EnterPageEventBtn").on('click', function () {
        var title = $(this).data('title');
        var name = $(this).data('name');
        var productItem = vm.productList[vm.currentProduct];
        var attr1 = productItem.productName || '';
        var attr2 = productItem.attr1 || '';
        typeof addPageEvent == 'function' && addPageEvent(title, name, attr1, attr2);//页面日志
    });
}
...
function initCanvasListener() {
    ...
    var main = document.getElementById('canvasBox');
...
function onUp(e) {
    ...
    if (bool) {
        bool = false;
        //typeof addPageEvent == 'function' && addPageEvent("首页", '首页-拖动');
    }
}