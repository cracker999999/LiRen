...
(function () {
    ...
    const configKey = "aiChatbotConfig";
    const buttonId = "ai-chatbot-bubble-button";
    const iframeId = "ai-chatbot-bubble-window";
    ...
    async function embedChatbot() {
        ...
        function createIframe() {
            ...
            iframe.title = "ai chatbot bubble window";
            iframe.id = iframeId;
        ...
        function createButton() {
            ...
            styleSheet.sheet.insertRule(`
                #${containerDiv.id} {
                    position: fixed;
                    bottom: var(--${containerDiv.id}-bottom, 2.7rem);
                    right: var(--${containerDiv.id}-right, 1.1rem);
            ...
            `);
            styleSheet.sheet.insertRule(`#${containerDiv.id}:hover {transform: var(--${containerDiv.id}-hover-transform, scale(1.1));}`);
            ...
            containerDiv.addEventListener("click", function () {
                ...
                // printElementSource(targetIframe);
                resetIframePosition();
            });
        ...
        function printElementSource(element) {
            if (element instanceof HTMLIFrameElement) {
                try {
                    // 尝试访问iframe内容
                    let iframeContent = element.contentDocument || element.contentWindow.document;
                    console.log(iframeContent.documentElement.outerHTML);
                } catch (error) {
                    if (error instanceof DOMException && error.name === "SecurityError") {
                        console.log("无法访问iframe内容：跨域限制");
                        console.log("iframe的src：", element.src);
                    } else {
        ...
        function enableDragging(element, axis) {
            ...
            function drag(e) {
                ...
                if (axis === "x" || axis === "both") {
                    // element.style.setProperty(`--${buttonId}-left`, `${Math.max(0, Math.min(newLeft, maxX))}px`);
                    const leftPercentage = (Math.max(0, Math.min(newLeft, maxX)) / window.innerWidth) * 100;
                    element.style.setProperty(`--${buttonId}-left`, `${leftPercentage}vw`);
                    element.style.setProperty(`--${buttonId}-right`, 'unset');
                }

                if (axis === "y" || axis === "both") {
                    // element.style.setProperty(`--${buttonId}-bottom`, `${Math.max(0, Math.min(newBottom, maxY))}px`);
                    const bottomPercentage = (Math.max(0, Math.min(newBottom, maxY)) / window.innerHeight) * 100;
                    element.style.setProperty(`--${buttonId}-bottom`, `${bottomPercentage}vh`);
                    element.style.setProperty(`--${buttonId}-top`, 'unset');
                }
            }
        ...
        //     if (button) {
        //       const leftVw = button.style.getPropertyValue(`--${buttonId}-left`);
        //       const bottomVh = button.style.getPropertyValue(`--${buttonId}-bottom`);
              
        //       if (leftVw) {
        //         const leftPx = (parseFloat(leftVw) / 100) * window.innerWidth;
        //         button.style.left = `${Math.min(leftPx, window.innerWidth - button.offsetWidth)}px`;
        //       }
              
        //       if (bottomVh) {
        //         const bottomPx = (parseFloat(bottomVh) / 100) * window.innerHeight;
        //         button.style.bottom = `${Math.min(bottomPx, window.innerHeight - button.offsetHeight)}px`;
        //       }
        //     }
        // });
